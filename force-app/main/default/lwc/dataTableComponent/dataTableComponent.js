...
export default class DataTableComponent extends LightningElement {
  ...
  handleSave(event) {
    // var newData = event.currentTarget.dataset.email;
    // console.log("save event: ", event.currentTarget.dataset.trip);
    ...
    var tripID, tagName, activity, note, textList, conEmail, tripDate, conName, oldActivity, actualMileage, mileage, chMileage;
    let tId = event.currentTarget.dataset.id;
    let rList = this.template.querySelector(
    ...
    tripID = tId;
    tagName = (rList.value === undefined) ? '' : rList.value;
    note = (textList.value === undefined) ? '' : textList.value;
    activity = (this.activity === undefined || this.activity === '') ? 'Business' : this.activity;
      ...
      .then((data) => {
        ...
        if (data) {
          ...
          if(this.isClicked){
            ...
            this.handlecloselookUp(event, tId, tagName, this.activity, note, chMileage)
          }else{
            this.handlecloselookUp(event, tId, tagName, this.activity, note)
          }
  ...
  handlecloselookUp(event, uId, tag, act, note, updateMileage) {
    // console.log('inside list', act, note, tag);
    var i, j;
    let targetId;
    if (uId != undefined) {
      targetId = uId;
    ...
    for (i = 0; i < j; i++) {
      ...
      if (
        ...
        if (targetId === closerow.dataset.id) {
          ...
          if (tag != undefined || act != undefined  || note != undefined) {
            if (closerow.previousElementSibling) {
              ...
              // console.log(closerow.previousElementSibling);
              closerow.previousElementSibling.cells[8].childNodes[0].textContent = tag;
              closerow.previousElementSibling.cells[9].childNodes[0].textContent = note;
              if(act != undefined){
                closerow.previousElementSibling.cells[10].innerText = act;
              }
              if(updateMileage != undefined){
                closerow.previousElementSibling.cells[5].innerText = updateMileage + ' mi';
              }
            }
          }
